...
| Module | Owns |
|---|---|
| `closed_form.py` | equations written once — an expression tree that evaluates, renders LaTeX and composes into models; the kernel's own cost laws, mirrored against `cost_model` by the test gate |
...
This table listed two of the five (now six) for some time; nothing verifies it, so it will drift again.
