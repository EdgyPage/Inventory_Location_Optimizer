...
_N_TYPES      = len(_AISLE_CFGS)
_REPLICAS     = 3
_TOTAL_AISLES = _N_TYPES * _REPLICAS
...
def _aisle_lift_sums(wh, affinity):
...
def build_shared_assets(log: logging.Logger) -> dict:
    """Build inventory, affinity, warehouse A, and load params (done once)."""

    # probe to measure exact bin count
    Carton.next_sku     = 1
    Aisle.next_aisle_id = 1
    random.seed(SEED_WORLD)
    probe      = Warehouse_Builder().from_config(WAREHOUSE_CFG).build()
    total_bins = len(probe.bins)
    ...
    log.info(f'Building sparse affinity  (max {_MAX_PER_GROUP} SKUs/group)...')
    t0       = time.perf_counter()
    affinity = inventory.affinity_matrix(max_per_group=_MAX_PER_GROUP)
    log.info(f'  {len(affinity):,} affinity entries  ({time.perf_counter() - t0:.2f}s)')
    ...
    # warehouse A: uniform assignment, built once and shared
    Aisle.next_aisle_id = 1
    random.seed(SEED_WORLD)
    ...
    log.info(f'Warehouse A (uniform): {placed_A:,} / {total_bins:,} bins filled  ({placed_A/total_bins:.1%})')
    ...
    log.info(f'Warehouse config: {_N_TYPES} aisle types × {_REPLICAS} replicas = {_TOTAL_AISLES} aisles')
    log.info(f'SKU catalog: {n_skus:,}  ({_FILL_TARGET:.0%} fill)  |  '
             f'Avg batch: ~{int(n_skus * _BATCH_MEAN_FRAC):,} SKUs')
    ...
    return dict(
        ...
        aisle_size_map     = aisle_size_map,
        aisle_unittype_map = aisle_unittype_map,
        aisle_handling_map = aisle_handling_map,
    )
...
def run_config(cfg: dict, shared: dict, base_dir: str, log: logging.Logger) -> None:
    ...
    la = np.array([_aisle_lift_sums(warehouse_A, affinity).get(a, 0.0) for a in range(1, _TOTAL_AISLES + 1)])
    lb = np.array([_aisle_lift_sums(warehouse_B, affinity).get(a, 0.0) for a in range(1, _TOTAL_AISLES + 1)])
    lc = np.array([_aisle_lift_sums(warehouse_C, affinity).get(a, 0.0) for a in range(1, _TOTAL_AISLES + 1)])
    ...
    for i in range(start_i, N_BATCHES):
        ...
        ea = PickSimulation(ta, pick_cfg).run()
        eb = PickSimulation(tb, pick_cfg).run()
        ec = PickSimulation(tc, pick_cfg).run()
        ...
        tsA = extract_task_stats(ea, ta, batch_id=i, affinity=affinity, wp=wp, run_id=run_a)
        tsB = extract_task_stats(eb, tb, batch_id=i, affinity=affinity, wp=wp, run_id=run_b)
        tsC = extract_task_stats(ec, tc, batch_id=i, affinity=affinity, wp=wp, run_id=run_c)
...
def main():
    parser = argparse.ArgumentParser(description='Warehouse assignment strategy comparison')
    parser.add_argument('--resume', metavar='BASE_DIR',
                        help='Resume a previous run by passing its base directory path')
    ...
    shared = build_shared_assets(log)
