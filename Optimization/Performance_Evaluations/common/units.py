...
# The one declaration of what the simulator counts in, and of the two days it counts in.
# `timeline` is stdlib-only, so importing it keeps this module's no-heavy-imports promise
# intact.  `SECONDS_PER_DAY` is re-exported below rather than aliased here, so the ~4 call
# sites that read it off THIS module keep working.
from Warehouse.kernel.timeline import SECONDS_PER_DAY, SECONDS_PER_HOUR
...
# ── the seconds->days divisor: NOT declared here ─────────────────────────────────
# `SECONDS_PER_DAY` is imported at the top of this module and re-exported under this
# module's name, which is why `frames._ydf` reading `units.SECONDS_PER_DAY` and
# `yard/scorecard` importing it from here both still work.  It used to be DECLARED on this
# line, and the simulation's own urgency gate then declared a second copy as a bare
# `86400.0` -- one knob, two readers, two literals, and nothing making them agree
# ("Pin the day divisor", inbound-optimization 30).  `timeline` now holds the one
# declaration, beside the SITE day, and carries the note on which is which.
#
# Why this suite wants the CALENDAR day: detention is what the carrier's trailer is held
# for, and it accrues overnight and at weekends exactly as it does mid-shift, so the site's
# working day would understate a standing trailer by however long the site is closed.  That
# CHOICE is this module's to make -- picking the scale is what a presentation-unit policy
# does -- and only the VALUE belongs to the kernel.
