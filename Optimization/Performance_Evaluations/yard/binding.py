...
@evaluation(key='yard.site_binding', label='Site yard binding (coupled)',
            scope='site', needs=('yard', 'batch'),
            family='yard', shape=('serial', 'ranked'),
            quantities=(DEPTH, CUTS))
def render_site(ctx, params):
    """Did the yard bind — the SITE's yard, for a coupled run.
