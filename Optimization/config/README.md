...
| `whatif_config.py` | `SPECS` — the cell matrices a run can be launched as; `ERA_RUN_DEFAULTS` / `PHASE2_RUN_DEFAULTS`, the run-level regimes the campaign specs default to; and the shape they are checked against (`validate_spec`, `channel_restocks_for`) |
