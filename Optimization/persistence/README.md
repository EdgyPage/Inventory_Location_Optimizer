...
| Module | Owns |
|---|---|
| `Picking_Data.py` | `sim_<arm>.db` — runs, batch/task stats, picker events, picks, the bin-mutation log (`bin_placement`/`bin_eviction`), keyframes, scores. Also the READER for the retired `bin_inventory`, kept so archived runs stay openable |
