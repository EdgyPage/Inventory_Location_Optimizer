...
def build_load_minimizing_assignment_fn(
    ...
    After selecting the best aisle a random compatible bin within it is returned.

    W_a is estimated as the full-aisle traversal cost
        W_a = x_move_time * bayXPerAisle + y_move_time * bayYPerAisle
    which is stable across placements and scales correctly with physical size.
    The per-item pick overhead is excluded because it is equal across all aisles
    for a given SKU and does not affect the relative ordering.
    ...
    Notes
    -----
    The closure owns mutable state (aisle_sku_sets, aisle_lift_sum) that is
    updated after every successful placement.  Pass a freshly built function to
    each Inventory_Manager instantiation; do not share across separate runs.
    """
    from collections import defaultdict as _dd
    import random as _rand

    # Pre-index affinity as sku → {partner: lift_value}.
    # Both (i,j) and (j,i) are present in `affinity`, so sku_partners[i][j]
    # stores the same value as sku_partners[j][i]; we use whichever direction
    # we happen to look up.  delta_lift = 2 * sum(v) because sum_lift counts
    # ordered pairs: affinity(s→si) + affinity(si→s) = 2 * affinity(s,si).
    sku_partners: dict[int, dict[int, float]] = _dd(dict)
    for (i, j), v in affinity.items():
        sku_partners[i][j] = v

    # Mutable placement state — updated on every successful assign() call
    ...
    def assign(unit: Any, candidates: list[Any]) -> Any | None:
        ...
        # Group candidates by aisle; remember one aisle object per aisle_id
        ...
        for aid, _bins in bins_by_aisle.items():
            ...
            # delta_lift: how much lift_sum grows when sku is added to this aisle.
            # Counts ordered pairs, so each undirected pair contributes 2×.
            delta_lift = 2.0 * sum(
            ...
            score = (delta_l2, old_L)   # tiebreak: prefer currently lighter aisle
            if score < best_score:
                best_score       = score
                best_aid         = aid
        ...
        # Commit state before returning so subsequent placements see the updated aisle
