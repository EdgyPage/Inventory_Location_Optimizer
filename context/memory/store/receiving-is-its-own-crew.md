...
metadata: 
  ...
  modified: 2026-08-25T23:42:34.872Z
---
...
**Where to look.** `Warehouse/inventory/dock.py`'s docstring carries all of the above plus
the three verified reasons a dock cannot be a `PutQueueSet` member.
`Warehouse/operations/unload.py` carries why there is no travel term, no speed and no mode
(and therefore why crew SIZE is the only lever). `Diagnostics/receiving_report.py` is the
only reader of `work_events` in the repo — five sabotage-verified cross-checks; run it after
any change to either stream.

Related: [[working-day-clock-plan-corrections]] (the day this rides on),
[[putaway-seams-for-inbound]], [[config-knob-has-five-seams]],
[[empty-bin-preference-is-structural]].
