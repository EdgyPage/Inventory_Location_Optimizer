...
- [Pool open was a tier rebuild](pool-open-was-a-tier-rebuild.md) — ticket 03: every evaluator pool open rebuilt the tier (45-54 s of a 65-72 s drain); FrozenTier per drain + TierSlice per open, byte-identical only because aisle/bracket order is recomputed from the FILTERED first appearance; the live aisle index cannot be reused (insertion-time ties)
