...
**How to apply:** To actually exercise reorder placement in a test, set `o.reorder_point = 0` on every order before `enqueue_all` (with 1-unit stock, one pick depletes → reorder fires; ~3000 placements over 40 batches). See `Tests/test_placement_fastpath_equivalence.py::_build_map_mgr`. This is a pre-existing coverage gap in `test_index_equivalence`, not specific to the map/cluster_map fast-path work.
