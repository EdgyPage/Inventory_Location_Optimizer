...
arch-synced-commit: a33d900f  <!-- updated by the architecture-maintainer agent after each sync; synced through the declared-once cut (a33d900f). Covers 6188def7..HEAD. Source commit 623e783e moved five values from "declared in two places that happen to agree" to one declaration the other side imports: DEFAULT_TARGET_FILL (inventory_planning), DEFAULT_ZONING_BANDS (inventory_zoning), DEFAULT_DOCK_DOORS (Inbound/transit), DEFAULT_FEE_THRESHOLD_DAYS + DEFAULT_URGENCY_HORIZON_DAYS (Inbound/gain). The new import edges run opt_config -> wh_inventory and opt_config -> inbound; both pass, because only the REVERSE directions are forbidden -- worth knowing before anyone "fixes" them. The interesting site was not a settings duplicate at all: n_bands had a fourth literal inside strategy_runner as a _zcfg.get fallback, where a stale default is invisible to every config seam. Three apparent duplicates were left alone deliberately (PutQueueSpec.swap_coef is a sentinel, BatchConfig.sampler is a recorded frozen-meaning decision, BatchConfig.mean_fraction matches FF by coincidence) and Tests/unit/test_declared_once.py pins all three so a later sweep cannot "finish the job". That test asserts IDENTITY rather than equality and was proven to fail under sabotage before being trusted. -->
