...
arch-synced-commit: 564f7e27  <!-- updated by the architecture-maintainer agent after each sync (context/arch/ + architecture.yml + files.yml + docs/architecture/); synced through site-dock ticket 12 "seat the put-pool injection seams" (564f7e27). New symbol catalogued: Inventory_Manager.count_put_cut, plus eleven new test functions in Tests/unit/test_putaway_timing.py and Tests/integration/test_putaway_day_cut.py; no new files, so no purpose fields to fill, and no architecture.yml backbone or layer edits -- the seams add no import and cross no boundary -->
