...
last-synced-commit: 1d712d389edf3534c71d227cac3c418df584725f  <!-- updated by the context-maintainer agent after each sync; synced through department-calibration ticket 35 "derive the fill headroom per bucket from the stationary fragmentation" (era_coverage.py adds derive_fill/derived_holds/stamp_fill/holds_at, called from fixed_point under the era (min_headroom not None) before plan_fn each round; inventory_planning.py's plan_warehouse gains bucket_hold= and declared_packing; flows/simulation.yml's build_shared_assets step anchors these plus the new guard Tests/unit/test_fill_headroom.py) -->
arch-synced-commit: 1d712d389edf3534c71d227cac3c418df584725f  <!-- updated by the architecture-maintainer agent after each sync (context/arch/ + architecture.yml + files.yml + docs/architecture/); synced through department-calibration ticket 35 "derive the fill headroom per bucket from the stationary fragmentation" (era_coverage.py adds derive_fill/derived_holds/stamp_fill/holds_at/floors_at/resolve_floors/declared_lines, stamp_fragmentation(frag=) called from fixed_point; inventory_planning.py's plan_warehouse gains bucket_hold= and declared_packing; new consts STORE_DEMAND/FF_DEMAND/FIRST_TIME_CONFIDENCE/MIN_HEADROOM@Optimization/config/settings.py; min_headroom/channel_demand/staffing_provenance@Optimization/config/sim_config.py; new file Diagnostics/equilibrium_report.py cataloged (pre-existing, missed by a prior sync); new test Tests/unit/test_fill_headroom.py cataloged) -->
