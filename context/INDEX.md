...
arch-synced-commit: 66515339  <!-- updated by the architecture-maintainer agent after each sync (context/arch/ + architecture.yml + files.yml + docs/architecture/); synced through site-dock ticket 16 "close the torn-finalize window" (66515339). New symbols catalogued: three test functions in Tests/integration/test_crash_recovery.py (test_a_kill_inside_finalize_leaves_the_dir_resumable, test_fresh_branch_refuses_to_create_a_second_run, test_fresh_branch_is_unchanged_on_an_empty_or_absent_db). No new files, so no purpose fields to fill. One new EDGE, not a new boundary: Optimization/simdriver/workunits.py now imports find_run from Optimization/persistence/Picking_Data.py, which it already imported create_run and init_run_db from -- no architecture.yml backbone/boundary edits -->
