...
last-synced-commit: 792b3df10ea57521ab690fd472ebdc430aafa8b4  <!-- updated by the context-maintainer agent after each sync; synced through 7 commits (ce9340ac..792b3df1, 2026-09-22) covering the plan-trace probe (Inbound/gain.py `_traced`/`plan_order_topm`, the `site_plan_trace` run-tree artifact written by `_SiteDock.flush_trace`/`arm_trace` in strategy_runner.py), the door-scarcity axis (whatif_config.py `door_scarcity_axis`/`PHASE2_DOOR_LEVELS`), and the FILL TRIAL driver mode (sim_config.py `fill_spec`; staffing.py `declared_work`/`derive_fill`/`fill_dispatch_rate`; workunits.py `_fill_payload`/`_channel_strategies`; strategy_runner.py `_FillDispatch`/`_build_fill` and the fill/pick stage loop in `_run_strategy_worker_impl`; Inventory_Management.py `declare_all`; supervisor.py copying `fill_batches` into sim_meta). ONE flow anchor set changed: `derive_staffing` (flows/simulation.yml) gained `fill_spec@Optimization/config/sim_config.py`, `declared_work@Optimization/simconfig/staffing.py`, and `derive_fill@Optimization/simconfig/staffing.py` (that step's `_derive_staffing_for_pair` now calls all three directly to price the fill's crews beside the steady-state ones) plus `Tests/unit/test_fill_trial.py` as a guard; the summary prose now names the fill trial. Everything else stays OUTSIDE flows/*.yml and artifacts.yml, consistent with existing practice: `_FillDispatch`/`_build_fill`/`_fill_payload`/`_channel_strategies`/`fill_dispatch_rate` are private helpers of already-anchored functions (`_run_strategy_worker`, `_build_work_units`, `_prepare_channel_run`) -- no anchor added per the "do NOT inventory private helpers" rule; `fill_batches` lands on the already-documented `sim_meta` artifact whose `fields` list already omits per-strategy nested keys (e.g. `expected_pick`), so no artifacts.yml edit. `site_plan_trace` is already a self-declared run-tree contract artifact (`Optimization/runschema/schema.py`, `writer: flush_trace@.../strategy_runner.py`) verified by `runschema/contract.py`, not by artifacts.yml -- same treatment as `site_inbound_db` and `unload_ranking_json` before it. Inbound/gain.py's plan-trace additions (`_traced`, `plan_order_topm`, `_SiteDock`/`arm_trace`), Inventory_Management.py's `declare_all`, and whatif_config.py's `door_scarcity_axis` sit in files (Inbound/gain.py, Warehouse/inventory/Inventory_Management.py, Optimization/config/whatif_config.py) that flows/*.yml has never referenced -- the gain evaluator, the placement/inventory layer, and the phase-2 campaign axes are all out of documented-pipeline scope here, same as `phase2_inbound_axis` before `door_scarcity_axis` joined it. `python context/verify_context.py` is green. -->
arch-synced-commit: 7bcd0cec  <!-- updated after each sync; synced through 4403a1ce..7bcd0cec (5 commits: 4c5ac3a4 bench_pool_open.py + its smoke test; f4666315 the plan-trace probe -- Inbound/gain.py plan_order_topm/TRACE_TOPM/bundle_uses_pool, strategy_runner's _SiteDock trace, the site_plan_trace artifact; f00ba000 yard figure labels + run_analysis._pair_fields; cde2bf7c door_scarcity_axis + Tests/unit/test_door_scarcity.py; 7bcd0cec the fill trial -- sim_config.fill_spec, staffing.declared_work/derive_fill/fill_dispatch_rate, workunits._fill_payload/_channel_strategies, strategy_runner _FillDispatch/_build_fill, Inventory_Management.declare_all, Tests/unit/test_fill_trial.py). Three new source/test files landed and were catalogued by --catalog-merge, all seeded with a real purpose from their own docstring (no purpose:TODO produced): Tests/bench/bench_pool_open.py ("price one gain-evaluator pool open in seconds, at the campaign shape"), Tests/unit/test_bench_pool_open.py ("the pool-open bench still measures what it says it measures"), Tests/unit/test_door_scarcity.py ("fewer doors is an understaffed dock, and its cells always have a twin"), Tests/unit/test_fill_trial.py ("the FILL TRIAL's driver mode (CONTEXT.md: Fill trial)"), Tests/unit/test_plan_trace.py ("the plan-trace probe records the exact plan and changes nothing"). All other changed files (Inbound/gain.py, Optimization/config/settings.py, sim_config.py, whatif_config.py, Optimization/simconfig/staffing.py, etc.) were already catalogued -- --catalog-merge only appended new key_symbols (plan_order_topm, TRACE_TOPM, bundle_uses_pool, INBOUND_PLAN_TRACE, INBOUND_FILL_SPAN_DAYS, INBOUND_FILL_RATIO, fill_spec, PHASE2_PLAN_TRACE_EVERY, PHASE2_DOOR_PROBE_LEVELS, PHASE2_DOOR_LEVELS, door_scarcity_axis, declared_work, derive_fill, fill_dispatch_rate) to existing entries; their human-owned purpose/notes were left untouched. architecture.yml needed no backbone/boundary edits: none of the new functions (plan_order_topm, the _FillDispatch/_build_fill pair, door_scarcity_axis, declare_all) sit on a curated backbone edge or a layer `members` list, and `verify_architecture.py` passed with zero spec changes. The full chain (extract --write [3958 nodes/4567 edges], --catalog-merge [512 entries], --write-nodes [3951 node details], render_html --build [4514 files]) was re-run and both verifiers (verify_architecture.py, verify_site.py full and --fast) are green on the first build -- no case-only rename this round. The previous note (ce9340ac) is superseded. -->
