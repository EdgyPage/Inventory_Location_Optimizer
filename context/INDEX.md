...
arch-synced-commit: 4066071a  <!-- updated by the architecture-maintainer agent after each sync (context/arch/ + architecture.yml + files.yml + docs/architecture/); synced through site-dock ticket 11 "harden the positional seams" (96adb0e6, 081ea109, 4066071a) AND the un-synced commits before it -- 8a5b4f42 "extract the standing drain into SiteReceiving" (site-dock 09) had not been synced, so this run regenerates both. New symbols catalogued: runlayout._reserved, ChannelRun.channel_key, workunits._stamp_identity, registry.SCOPES, requests.REQUEST_SCOPE, and three new test modules (test_reserved_prefix_walk, test_unit_identity_is_carried, test_evaluation_scope) whose purposes were hand-filled rather than left as the merge's docstring fragment; no architecture.yml backbone or layer edits -- none of the new symbols sits on a documented backbone edge or crosses a layer boundary -->
