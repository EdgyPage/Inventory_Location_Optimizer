...
arch-synced-commit: 2d47d903  <!-- updated by the architecture-maintainer agent after each sync; synced through the ShiftLedger derived re-sync (2d47d903). Covers ea800424..HEAD in FIVE windows: (1) 54d6c102..245a173b + d279c7f6 -- six files, anchor re-resolution WAS needed (ZoningMixin, the six SiteReceiving.receive steps, _build_parser, the _CowView base); (2) 4af16744..f454b1d6 -- the digest-surface gate, no symbol moved; (3) 6c52411e..6e469a54 -- section_timers.CheckpointWindow; (4) e66d55fe..acb51098 -- RUNTIME_TABLES; (5) 0fcd423d..2d47d903 -- shift_ledger.ShiftLedger, replacing four _build_leaf closure variables. architecture.yml needed no edit in any window. Every new purpose arrived AUTO-FILLED from a module docstring rather than as TODO; in window (5) the module and its TEST auto-filled to the SAME string, so check for duplicates when hand-filling a pair. A test_merge_is_idempotent failure is not automatically the multi-line hand-fill trap -- diff a fresh --catalog-merge first. Tests/architecture drift is UNCHANGED at the same five throughout, PROVEN against a git-archive copy of fa9373af. -->
