...
arch-synced-commit: 9b0e21e8  <!-- updated by the architecture-maintainer agent after each sync (context/arch/ + architecture.yml + files.yml + docs/architecture/); synced through site-dock ticket 19 "build the site put-away pool and the site put clock" (9b0e21e8). New file Inbound/putaway_pool.py (PutawayPool: the site put pool -- one shared clock list over both channels' put queues, the day's proportional division, the residue re-drain, the once-per-site-day reset and the site put carry) and new test file Tests/unit/test_putaway_pool.py; both purposes were written by hand because --catalog-merge seeded them from a docstring FRAGMENT again (11 and 18's finding, reproduced a third time) -- and a MULTI-LINE purpose breaks test_merge_is_idempotent, so both are one line. New symbols in strategy_runner.py: _work_day_of, _build_put_pool, and _build_leaf._replenish (the batch's replenishment half, split out of _step); ReorderMixin._drain_putaway is now the public drain_putaway. No architecture.yml change: Inbound -> wh_kernel was already allowed and the curated backbone names no edge that moved. render_html --build hit an OSError on the first pass and was re-run (memory case-only-rename-deletes-its-own-page) -->
