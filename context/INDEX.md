...
arch-synced-commit: 4ecdf43d  <!-- updated by the architecture-maintainer agent after each sync (context/arch/ + architecture.yml + files.yml + docs/architecture/); synced through site-dock ticket 13 "seat the one-owner bundle indirection" (4ecdf43d). New symbol catalogued: OneOwnerBundle in Inbound/gain.py (key_symbols on Inbound/gain.py), plus four new test functions in Tests/unit/test_gain_plan.py (test_one_owner_hands_back_the_same_instance_for_every_key, test_a_bare_bundle_is_refused_rather_than_sniffed_for, test_the_cursor_tracks_the_group_being_priced, test_a_provider_that_answers_one_key_differently_moves_the_price); no new files, so no purpose fields to fill, and no architecture.yml backbone/boundary edits -- the Optimization.simdriver -> Inbound.gain import already existed, only the wrapped symbol at the injection site changed -->
