...
arch-synced-commit: 6a4428d5  <!-- updated after each sync; synced through 53964420..6a4428d5 (11 commits): the flat-pool measurement/pin docs, the placement-score (pick_owed) metric replacing the tied flow-total ranking in run_unload_ranking.py, the cell_is_complete/in_flight resume fix (cells.py's `_cell_complete` deleted, replaced by RunTree.cell_is_complete + RunTree.in_flight in runschema/resolver.py; scripts/archive_cells.py's `_in_flight` deleted, now delegates), duration-weighted dispatch (new opt_simdriver module pacing.py: load_pace/weight_of/weigh_jobs/cell_order, wired behind --pace-from), and the run_history/run_index ledgers (sim_manifest.py gained open_run_record/close_run_record/read_run_history/append_run_index/read_run_index; new scripts/run_index.py CLI reads run_index.jsonl). Catalog gained 8 entries: Optimization/simdriver/pacing.py, Optimization/Performance_Evaluations/layout/pick_owed.py, scripts/run_index.py, Tests/unit/test_pacing.py, Tests/unit/test_cell_is_complete.py, Tests/unit/test_run_history.py, Tests/unit/test_pick_owed.py, Tests/unit/test_pick_owed_agrees_with_the_closed_form.py. No backbone/boundary edits were needed -- architecture.yml's existing claims already held against the regenerated graph. The full chain (extract --write, --catalog-merge, --write-nodes, render_html --build) was re-run and both verifiers are green on the first build (no case-only rename this round). The previous note (53964420) is superseded. -->
