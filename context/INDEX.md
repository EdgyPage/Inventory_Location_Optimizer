...
arch-synced-commit: 323a256b  <!-- updated after each sync; synced through architecture-deepening ticket 07 (34fbc392 CheckpointBuffer, 323a256b the writer collapse). Covers 1e321745..HEAD -- a wide window: tickets 02B, 03, 04, 06, 07, 11, 16, 17, 20, 22, 23 all landed in it and the full chain (extract --write, --catalog-merge, --write-nodes, render_html --build) was re-run after each, so the derived layer matches HEAD. verify_architecture and verify_site --fast both green. This window DELETED 15 Picking_Data writers and moved _shift_close_out out of ArmAssembly to module level, so any anchor naming save_<table>@Picking_Data or ArmAssembly.shift_close_out is stale by construction -- none remained outside files.yml, which is regenerated. -->
