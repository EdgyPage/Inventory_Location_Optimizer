...
arch-synced-commit: 242bf96d  <!-- updated by the architecture-maintainer agent after each sync; synced through "chore(arch): re-sync the architecture layer and the run-tree fingerprint" (242bf96d). Covers commits ea800424..242bf96d (fourteen commits). The shape-changing ones are the five-commit refactor 54d6c102..245a173b plus the test-only d279c7f6: SIX new files entered the catalog -- Optimization/simdriver/section_timers.py, Warehouse/inventory/inventory_zoning.py, and four Tests/unit files (test_section_timers, test_gain_cow_protocol, test_config_reaches_the_worker, test_cli_surface). Their `purpose` arrived AUTO-FILLED from the module docstring rather than as `TODO` (the known catalog-merge behaviour), so all six were rewritten by hand. Anchor re-resolution WAS needed this time, unlike the previous window: eight zoning methods moved from Inventory_Manager to the new ZoningMixin, six step methods were extracted from SiteReceiving.receive, _build_parser was split out of run_simulation.main, and the three _Cow* views gained a _CowView base -- all picked up by a full extract chain (--write, --catalog-merge, --write-nodes, render_html --build). architecture.yml needed no edit: no symbol named in a backbone edge, hotpath or layer members list moved. Pre-existing Tests/architecture drift is UNCHANGED at the same five as the prior sync, PROVEN by running the tier against a `git archive` copy of fa9373af: baseline 9 failed / 375 passed, HEAD 5 failed / 379 passed, and HEAD-minus-baseline is empty -- the four extra baseline failures are the git-dependent ones an archive copy cannot satisfy (test_mirror_is_actually_tracked_by_git, test_every_tracked_file_is_clean, test_integrity_committed_tree_matches_manifest, test_reference_extraction_is_not_vacuous). -->
