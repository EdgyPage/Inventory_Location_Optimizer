...
arch-synced-commit: 1e321745  <!-- updated after each sync; synced through the vol-sum teardown fix (1e321745). Covers 1b8123ee..HEAD. NOTE the previous marker had gone stale by several commits -- 45e7728f re-synced the derived layer without bumping it -- so this window is wider than one session's work. The claim is mechanical rather than reviewed: the full chain (extract --write, --catalog-merge, --write-nodes, render_html --build) was re-run from the current source tree, so the derived layer matches HEAD whatever happened in between; verify_architecture and verify_site --fast are both green. Only change in this window's own work: Tests/unit/test_aisle_vol_sum_teardown.py catalogued (Tests/ is catalog-only, so graph.json did not move) and ReorderMixin._drop_sku_from_aisle gained a fourth decrement. -->
