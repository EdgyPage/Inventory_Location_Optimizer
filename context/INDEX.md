...
arch-synced-commit: 6188def7  <!-- updated by the architecture-maintainer agent after each sync; synced through the AuditLedgers/lap-cursor derived re-sync (6188def7). Covers ea800424..HEAD. The _build_leaf STATE PROGRAMME is the through-line: 62 closure variables became 24, via five objects -- section_timers.SectionTimers (23 names), section_timers.CheckpointWindow (6), shift_ledger.ShiftLedger (4), audit_ledgers.AuditLedgers (4) and the lap cursor (1). Each was chosen because a RULE lived in the shape it replaced, not because the count was high. The remaining 24 are deliberate: 11 teardown references whose `nonlocal` is load-bearing (del needs it, and the del is what lets gc.collect reclaim the cyclic warehouse graph), a 5-name batch frame carrying no rule, 3 crew clocks read per-batch, and small tallies. architecture.yml needed no edit in any window. Every new purpose arrives AUTO-FILLED from a module docstring rather than as TODO, and a module and its TEST often auto-fill to near-identical strings -- check for duplicates when hand-filling a pair. A test_merge_is_idempotent failure is not automatically the multi-line hand-fill trap: diff a fresh --catalog-merge first. Tests/architecture drift is UNCHANGED at the same five throughout, PROVEN against a git-archive copy of fa9373af. -->
