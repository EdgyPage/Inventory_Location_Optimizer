...
arch-synced-commit: 68b262de  <!-- updated by the architecture-maintainer agent after each sync; synced through site-dock ticket 21, the coupled receiving coordinator (68b262de). No new source FILE this time -- `SiteReceiving` grew its coupled half (SITE_PHASES, the {sku: leaf} owner dict, bind/open_batch/note_records/drain_site_rows/_owner_of/_leaf_for), `Inventory_Manager` gained the class attribute `site_scoped` and the `_refuse_site_scope` helper behind six leaf accessors, and `ReorderMixin` gained the third public port `owned_skus`. `--catalog-merge` therefore seeded no `purpose` at all (the docstring-fragment trap needs a new ENTRY to bite; memory catalog-merge-seeds-a-docstring-fragment still stands for the next new file). What it did add is the new test functions plus SITE_SCOPED/SITE_PHASES; `Inbound/receiving.py`s curated `notes` was rewritten BY HAND -- it still said "two public ports". No architecture.yml change: inbound -> wh_kernel was already allowed and no curated backbone edge names a function that moved -->
