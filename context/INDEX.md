...
arch-synced-commit: b568042a  <!-- updated by the architecture-maintainer agent after each sync; synced through site-dock tickets 20, 22 and 23, built in PARALLEL and committed as 03dbf3ab (the site space view), 6a8225ef (the coupled resume reconciler) and b568042a (the funnel spec as rule pairs). One new source file, Inbound/site_space.py (compose_site_view), plus four new test modules. `_run_whatif_matrix` lost two branches no input could reach, and `whatif_config.PHASE2_ARMS` became `PHASE2_PAIRS` -- the catalogue`s key_symbols for that file moved with it. All five new `purpose` fields were written by hand: --catalog-merge seeded every one from a docstring FRAGMENT (the fourth occurrence; one was cut off mid-sentence), and a MULTI-LINE purpose additionally breaks test_merge_is_idempotent, so each is one long line. Now a memory: catalog-merge-seeds-a-docstring-fragment. No architecture.yml change -- inbound -> wh_kernel was already allowed and no curated backbone edge names a function that moved -->
