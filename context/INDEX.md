...
arch-synced-commit: 6e469a54  <!-- updated by the architecture-maintainer agent after each sync; synced through "chore(arch): re-sync the derived layer for CheckpointWindow" (6e469a54). Covers ea800424..HEAD. THREE windows in one note: (1) the five-commit refactor 54d6c102..245a173b plus d279c7f6 added six files and DID need anchor re-resolution -- eight zoning methods to the new ZoningMixin, six step methods out of SiteReceiving.receive, _build_parser out of run_simulation.main, a _CowView base under the three _Cow* views; (2) the audit-fix window 4af16744..f454b1d6 added Tests/architecture/test_digest_surface.py and moved no symbol; (3) 6c52411e..HEAD added Optimization/simdriver/section_timers.CheckpointWindow and Tests/unit/test_checkpoint_window.py, replacing six _build_leaf closure variables. architecture.yml needed no edit in any of the three: nothing named in a backbone edge, hotpath or layer members list moved. Every new purpose arrived AUTO-FILLED from a module docstring rather than as TODO (the known catalog-merge behaviour) and was rewritten by hand. Tests/architecture drift is UNCHANGED at the same five throughout, PROVEN against a git-archive copy of fa9373af: baseline 9 failed/375 passed, HEAD 5 failed/391 passed, HEAD-minus-baseline empty -- the four extra baseline failures are the git-dependent ones an archive copy cannot satisfy. -->
