...
last-synced-commit: 5396442051434ff3aef36c01cf29fc2b38417529  <!-- updated by the context-maintainer agent after each sync; synced through the flat work-pool refactor (5 commits, all 2026-09-19, ending at 53964420 "every cell's three stages through the one work pool"): simulation.yml's run_workers step now names the real chain `_run_whatif_matrix -> _run_cells -> {_build_assets/build_shared_assets, _build_work_units, sim_jobs} -> WorkPool (submit/absorb/finish) -> _run_strategy_worker`, replacing the deleted `_run_scenario/_run_workers_flat/_supervise/_run_pool`; added anchors cells.cell_scope, workpool.WorkPool, workpool._unblock_broken_pool, supervisor.sim_jobs, supervisor._sim_executor, scenario._run_whatif_matrix/_run_cells/_build_assets, plus guards Tests/integration/test_work_pool.py and Tests/unit/test_cell_scope.py. analysis.yml needed no change (it never named run_analysis.run_analysis per cell or _drain). runschema/schema.py's run_manifest note already named scenario._build_assets. -->
