...
arch-synced-commit: 1b8123ee  <!-- updated by the architecture-maintainer agent after each sync; synced through the complexity-round instrument fixes (1b8123ee). Covers a33d900f..HEAD. Three new functions in Tests/calltree/calltree_growth.py: _flows_warning (the ALL-ZERO warning was the else of the per-ENTRY-CALL branch, so it fired on every non-inbound rung and advised switching to the config already in use), _severity_sort and _project (offenders ranked by bare exponent put a 92 ms section fitted off a 0.05 ms anchor above every real finding). Nothing under Warehouse/, Inbound/ or Optimization/ moved in this window -- the measured R x A finding (k=1.912) is recorded in .scratch/complexity-round/ and NOT yet acted on, deliberately: it is worth 2.66% of a rung today and its dominance at scale is a prediction the deep ladder has still to test. -->
