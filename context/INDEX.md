...
arch-synced-commit: 3c54866b  <!-- updated by the architecture-maintainer agent after each sync (context/arch/ + architecture.yml + files.yml + docs/architecture/); synced through site-dock ticket 17 "exempt repack rows and pin the unload constant" (fe80ed92 + 3c54866b). New symbols catalogued: five test functions in Tests/e2e/test_receiving_e2e.py (test_a_repack_row_is_exempt_but_the_clause_still_bites, the varied_arm fixture, test_the_unload_price_collapses_to_the_config_s_own_constant, test_the_constant_check_catches_its_three_defects, test_an_empty_sku_scores_makes_the_check_inactive_not_green). No new files, so no purpose fields to fill. No new edges and no architecture.yml change: Diagnostics/receiving_report.py already imported Optimization.persistence, and check 6's _unload_price is private so it does not enter key_symbols -->
