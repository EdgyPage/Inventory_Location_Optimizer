...
arch-synced-commit: acb51098  <!-- updated by the architecture-maintainer agent after each sync; synced through "chore(arch): re-sync the derived layer for RUNTIME_TABLES" (acb51098). Covers ea800424..HEAD in FOUR windows: (1) the five-commit refactor 54d6c102..245a173b plus d279c7f6 -- six new files, and anchor re-resolution WAS needed (eight zoning methods to the new ZoningMixin, six step methods out of SiteReceiving.receive, _build_parser out of run_simulation.main, a _CowView base under the three _Cow* views); (2) the audit-fix window 4af16744..f454b1d6 -- Tests/architecture/test_digest_surface.py, no symbol moved; (3) 6c52411e..6e469a54 -- section_timers.CheckpointWindow and its test, replacing six _build_leaf closure variables; (4) e66d55fe..acb51098 -- RUNTIME_TABLES and three gate tests. architecture.yml needed no edit in any window: nothing named in a backbone edge, hotpath or layer members list moved. Every new purpose arrived AUTO-FILLED from a module docstring rather than as TODO and was rewritten by hand. NOTE for the next sync: a test_merge_is_idempotent failure is not automatically the multi-line hand-fill trap -- diff a fresh --catalog-merge against the committed file first; in window (4) it was simply four un-catalogued symbols. Tests/architecture drift is UNCHANGED at the same five throughout, PROVEN against a git-archive copy of fa9373af: baseline 9 failed/375 passed, HEAD 5 failed/397 passed, HEAD-minus-baseline empty -- the four extra baseline failures are the git-dependent ones an archive copy cannot satisfy. -->
