...
arch-synced-commit: 6bcd207273769b9c9442a0bc22fae9b90c2e1a14  <!-- updated by the architecture-maintainer agent after each sync (context/arch/ + architecture.yml + files.yml + docs/architecture/); synced through inbound-optimization ticket 27 "chain the supplier lead before the trailer" (Inbound/transit.py's TrailerTransit gains _release_site/_load/_site_qty/_site_rows and an _at_site queue so dispatch serves the SKU's supplier lead before the trailer loads it; era_coverage.py drops refuse_discarded_lead now that the lead is chained rather than discarded; new test Tests/unit/test_supplier_lead_queue.py cataloged; test_lead_aware_coverage.py's discard-refusal test replaced by test_an_lt1_sibling_declares_at_its_supplier_lead_plus_the_transit) -->
