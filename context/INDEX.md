...
arch-synced-commit: f454b1d6  <!-- updated by the architecture-maintainer agent after each sync; synced through "chore(arch): catalogue the digest-surface gate and re-sync the derived layer" (f454b1d6). Covers ea800424..f454b1d6. Two windows in one note: the five-commit refactor 54d6c102..245a173b plus d279c7f6 added SIX files and DID need anchor re-resolution (eight zoning methods to the new ZoningMixin, six step methods out of SiteReceiving.receive, _build_parser out of run_simulation.main, a _CowView base under the three _Cow* views); the follow-up window 4af16744..f454b1d6 added a SEVENTH file, Tests/architecture/test_digest_surface.py, and moved no symbol. architecture.yml needed no edit in either: nothing named in a backbone edge, hotpath or layer members list moved. All six/seven new purposes arrived AUTO-FILLED from module docstrings rather than as TODO (the known catalog-merge behaviour) and were rewritten by hand. Tests/architecture drift is UNCHANGED at the same five as the prior sync, PROVEN against a `git archive` copy of fa9373af: baseline 9 failed/375 passed, HEAD 5 failed/385 passed, HEAD-minus-baseline empty -- the four extra baseline failures are the git-dependent ones an archive copy cannot satisfy. -->
