...
arch-synced-commit: 20ba2fcd3a47ff7a647e09cffddbeff3894247a1  <!-- updated by the architecture-maintainer agent after each sync (context/arch/ + architecture.yml + files.yml + docs/architecture/); synced through inbound-optimization ticket 28 "cap the door team" (0a441095 -- Warehouse/inventory/inventory_reorder.py's _unload_split/_unload_merged gain a cap parameter and _receive reads transit.door_team; Inbound/transit.py's YardTransit gains a door_team slot/kwarg; Optimization/config/settings.py's INBOUND_DOOR_TEAM and sim_config.py's inbound_door_team/INBOUND_KEYS/inbound_spec() door_team field; whatif_config.py's PHASE2_DOOR_TEAM and phase2_inbound_axis(door_team=); run_simulation.py's --inbound-door-team flag; run_analysis.py stamps inbound_door_team/inbound_dock_doors; strategy_runner.py passes door_team= to YardTransit; EvalContext.dock_ceiling() new; yard/scorecard.py's _receiver_busy() and two new columns; throughput/audit.py's receiving row gains a ceiling suffix; new test Tests/unit/test_door_team_cap.py cataloged, purpose seeded from its module docstring, no manual fill needed; no architecture.yml backbone/layer edits -- none of the new symbols sit on a documented backbone edge or layer boundary) -->
