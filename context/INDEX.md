...
arch-synced-commit: 22e41e4f  <!-- updated after each sync; synced through the I/O round and the resume fixes (145800bb..22e41e4f: deferred indexes, the held checkpoint connection, save_tail, scan_width, the resume idempotency fixes) plus the working tree that became the store_index --check gate commit. graph.json had been kept fresh by those commits without this note moving; the full chain (extract --write, --catalog-merge, --write-nodes, render_html --build) was re-run here and both verifiers are green. The previous note (323a256b, architecture-deepening ticket 07) is superseded. -->
