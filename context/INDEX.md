...
arch-synced-commit: f97b5e3d  <!-- updated by the architecture-maintainer agent after each sync; synced through inbound-optimization ticket 34, "compose the futuresight window at the site dock" (f97b5e3d). Covers commits 156970ce..f97b5e3d (the v3 sampler/draw-probability work, the funnel window/arrival-regime/staffing-pin declarations, the SECONDS_PER_DAY split, the campaign-cell coupling gate, and the futuresight window zip). New files catalogued: Tests/unit/test_campaign_cells_can_run.py (joins POLICY_VIEW_NEEDS against COMPOSED_VIEW_FIELDS/UNCOMPOSED_VIEW_FIELDS via Inbound.site_space.uncomposable_policies to pin that every declared campaign cell is one the coupled run can actually run) and Tests/unit/test_funnel_window.py (pins the campaign window/arrival-regime/staffing-pin as single declarations, and the site-day vs calendar-day unit split), both filled BY HAND after --catalog-merge seeded a docstring-fragment purpose. Re-anchored: ticket 34 replaced test_site_space_view.py::test_a_futuresight_window_is_refused_rather_than_zipped (the window is now composed, not refused) with seven successor tests (test_the_windows_zip_by_batch_index_and_union_disjoint_skus, test_the_composed_depth_is_the_window_depth_not_the_sum, test_a_sku_in_both_windows_is_refused, test_windows_of_different_depths_are_refused, test_a_window_on_one_leaf_only_is_refused, test_no_feed_composes_to_none_and_two_empty_windows_to_empty, test_the_generic_refusal_still_fires_when_the_declaration_is_empty) -- files.yml's key_symbols list updated to match; the file's human-owned purpose line needed no change. No architecture.yml change: no backbone/layer anchor named a symbol that moved. -->
