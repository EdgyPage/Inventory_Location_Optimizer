...
before reserve/pallet).

**Tasks → pickers (the scheduler).** The tasks are then handed to the $K$ pickers by the run's
**scheduler** — the lever this experiment sweeps. The reference cell dispatches **round-robin by
aisle**; the treatment cell uses **LPT** (longest-processing-time) load-balancing, giving each task
to the picker who will finish earliest. The scheduler changes only *when* each task-second is worked,
so it moves the [batch makespan](glossary.md#makespan) (and throughput) at **unchanged** total [task
makespan](glossary.md#task-makespan) (labor). See the [Comparison](comparison.md) for the A/B result.
...
| Varied | Levels |
|--------|--------|
| **Picker scheduler** (the what-if axis) | `round_robin` (cell `k1_off_rr`) · `lpt` (cell `k1_off_lpt`) |
| Replenishment lead time | {% for key, inv in experiment().inventories.items() %}`{{ key }}`{% if not loop.last %} · {% endif %}{% endfor %} |
| Pick-time calibration (channel) | {% for c in experiment().configs %}`{{ c.name }}`{% if not loop.last %} · {% endif %}{% endfor %} |
| Placement strategy | initial layout {uni, opt} × 17 restock families (see [Formula reference](formula-reference.md#the-families)) |

The scheduler is the **cross-cell** axis (its delta is the headline); the other three vary *within*
each cell. See the [experiment overview](index.md) for how the two schedulers compare.
