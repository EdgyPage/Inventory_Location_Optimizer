...
!!! note "Notes"
    The cost model and assignment-function objectives below are **unchanged from Experiment 4** —
    this experiment varies the *picker scheduler*, not the pick model. A scheduler only reorders how
    the same per-aisle tasks are dealt to pickers, so it changes [batch makespan](glossary.md#makespan)
    (throughput) at **unchanged** [task makespan](glossary.md#task-makespan) (labor): every per-pick
    and per-task formula on this page is identical across the two cells. What the result pages add is
    the **modeled-hours** conversion below.
...
The simulation records every duration in **milliseconds**; the result pages report **hours**, a
straight unit conversion (`Optimization/run_whatif_labor.py`):

$$\text{total labor hours} = \frac{\sum_{\text{batches}} \text{task makespan (ms)}}{3\,600\,000},
\qquad
\text{batch-time hours} = \frac{\sum_{\text{batches}} \text{batch makespan (ms)}}{3\,600\,000},$$

and throughput in items/hour is `thr_batch × 3 600 000`. The **labor hours saved** by an assignment
function is its total labor hours below the FIFO arm's, within the same scheduler / channel /
inventory (`saved = labor(fifo) − labor(arm)`, **+ = less labor**).

!!! warning "Modeled, not wall-clock"
    These hours are **modeled sim pick-time** — the pick-time cost model above, summed and rescaled.
    They are a comparable *effort* figure across arms and schedulers, **not** an elapsed wall-clock
    time or a staffing/labor-cost estimate. Only the [batch-time hours](glossary.md#makespan)
    correspond to how long a batch takes; the [total labor hours](glossary.md#task-makespan) are the
    serial sum a single picker would incur.
