...
!!! note "Why a synthetic warehouse, and what that buys"
    ...
    strongest evidence for that transfer so far is internal: the same winners and signs survived a
    complete change of catalogue between [Experiment 6](../experiment-6/index.md) and this run.
...
round-robin by aisle in `k1_off_rr`, longest-task-first in `k1_off_lpt`.
