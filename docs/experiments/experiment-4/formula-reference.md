...
!!! note "Notes"
    The cost model and assignment-function objectives below are **unchanged from Experiment 3** —
    this experiment varies the *warehouse geometry* (aisle-split, zoning), not the pick model. The
    one thing the split changes is the realised travel term $T$: shorter aisles mean a smaller
    per-visit x-sweep, which is why throughput moves while the per-pick formula stays fixed.
